...
class SimpleZombieStrategy(OurHumanStrategy):
    def decide_moves(
        ...
        for [character_id, moves] in possible_moves.items():
            ...
            closest_human_distance = (
                1984  # large number, map isn't big enough to reach this distance
            )
            ...
            for c in game_state.characters.values():
                ...
                if (
                    distance < closest_human_distance
            ...
            for m in moves:
                distance = abs(m.destination.x - closest_human_pos.x) + abs(
                    m.destination.y - closest_human_pos.y
